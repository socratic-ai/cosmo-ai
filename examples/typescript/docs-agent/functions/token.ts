import { COSMO_DEFAULT_BASE_URL, bearer, checkPassword, type Env } from './_auth';
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  // With no secrets configured (`wrangler pages dev` against a local .env),
  // the caller's own value is the key — pasting a workspace key into the
  // password box mints against it.

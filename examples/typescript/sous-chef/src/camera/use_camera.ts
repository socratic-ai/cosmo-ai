 ...
 * camera-kind track, flip, and tear down. Defaults to the rear lens so a phone
 * propped against the backsplash streams the pan, not the cook.
 *
  ...
  const release = useCallback(async (): Promise<void> => {
    ...
    if (handle !== null && session !== null) {
      try {
        ...
        console.error('[sous-chef] removeVideoStream failed', err);
      }
    }
